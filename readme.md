...
## Поддержка сборщика прекращена!

Теперь есть сборщик [Jossy](https://github.com/Kolyaj/Jossy), имеющий расширенную функциональность, но и немного другую семантику, поэтому вынесенный в отдельный проект.

Отличия в семантике заключаются в подключении областей файлов. В BuildJS все области файла, нужные для текущей сборки, подключаются при первом подключении файла. В Jossy же каждая область ведёт себя самомстоятельно и в последующем порядок кода может измениться.

Если вы ничего не поняли, то простые правила: 

1. Если вы только хотите начать использовать сборщик, то возьмите Jossy, он лучше.
2. Если вы использовали BuildJS, но не использовали `#label`, то смело переезжайте на Jossy, у вас ничего не сломается.
3. Если же вы использовали `#label`, значит вы знали, что делали. После переезда хорошенько протестируйте ваш проект.
